---
title: "8.3 Who to follow"
output: word_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## R Markdown

This is an R Markdown document. Markdown is a simple formatting syntax for authoring HTML, PDF, and MS Word documents. For more details on using R Markdown see <http://rmarkdown.rstudio.com>.

When you click the **Knit** button a document will be generated that includes both content as well as the output of any embedded R code chunks within the document. You can embed an R code chunk like this:

```{r}
"retweet_retweet_count"
library(rtweet)
library(twitteR)
key = "COghWIH9VbXOtQI6m9xrSbOVN"
secret = "f0M4L0oMsvGpxcdb7MycEwxyyJC7Hub29hjnVUTEUO7oKeufsx"
access_tokens = "2388475730-lZ4Yqs8Jd3XtKZZQoTOz3wcYjypHbbnHKjwAQE1"
access_secret = "uHZHaALuELTlITgvJXR7PuG1pYGk8297RNGr8cUspO2C9"
setup_twitter_oauth(key, secret, access_tokens, access_secret)


#11
tweets = readRDS("tweets.rds")
attach(tweets)
tweets[1:100, ]
top100.tweets <- tweets[order(retweet_count, decreasing=TRUE)[1:100] ,  ] 
print(top100.tweets)

#12
UsersOfTweets=users=top100.tweets[1:100, c("screen_name")]
print(users)

#13
Users <- lookupUsers(c(UsersOfTweets))
followerCounts = sapply(Users, function(x) x$followersCount)
statusesCounts = sapply(Users, function(x) x$statusesCount)

print (followerCounts)
print (statusesCounts)

#14
chisq.test(followerCounts)
chisq.test(statusesCounts)

test <- chisq.test(followerCounts, statusesCounts)
test





```

## Including Plots

You can also embed plots, for example:

```{r pressure, echo=FALSE}
plot(pressure)
```

Note that the `echo = FALSE` parameter was added to the code chunk to prevent printing of the R code that generated the plot.
